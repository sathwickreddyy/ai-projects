...
1. **MCP Server** (`mcp-server/server.py`) — stdio transport, FastMCP. 8 tools: `check_app_health`, `push_architecture`, `pull_latest_state`, `list_sessions`, `get_skill_tree`, `save_skill_adaptation`, `install_skill`. `get_skill_tree` and `save_skill_adaptation` read/write the LOCAL project's skill files (`.claude/commands/intelligent-arch-creator/`). Other tools pass through to backend at `http://localhost:18000`.
